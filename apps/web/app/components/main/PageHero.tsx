    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-[#0c4a6e] via-[#0369a1] to-[#0ea5e9] text-white">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 text-center">
        <motion.span
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/20 backdrop-blur-sm text-white text-sm font-medium rounded-full mb-4"
        >
          {badgeIcon ? (
            <span className="inline-flex items-center">{badgeIcon}</span>
          ) : null}
          <span>{badge}</span>
        </motion.span>
        ...
        <motion.h1
          ...
          {highlight ? (
            <>
              {" "}
              <span className={accentColor}>{highlight}</span>
            </>
          ) : null}
        </motion.h1>
        <motion.p
          ...
          <motion.div className="mt-8 flex flex-wrap justify-center gap-3">
            {stats.map((stat, i) => (
              ...
              >
                ...
                {stat.label ? (
                  <motion.p className="text-xs text-slate-300 mt-0.5">
                    {stat.label}
                  </motion.p>
                ) : null}
              </motion.div>
